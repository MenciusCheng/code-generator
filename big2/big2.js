(function () {
    ...
    let initSql = "@=[datasource,isuwang_im]\n@=[apiPackage,im.message]\n@=[servicePackage,im]\n"
    $('#sqlTextArea').val(initSql)

    // 自定义模板
    let big2DiyArray = ['big2-diy1', 'big2-diy2', 'big2-diy3']
    $('#templateTextArea').bind("change", function (event) {
        let selectValue = $('#templateSelect').val()
        if (big2DiyArray.some(a => a == selectValue)) {
